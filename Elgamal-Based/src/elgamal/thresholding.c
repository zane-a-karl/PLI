    ...
    InputArgs             ia)
{
    ...
    /* r = iteratively_check_all_subsets(matches, secret_digest, shares, ia, server_keys.pk->modulus); */
    /* if (!r) { return general_error("Failed during iteratively_check_all_subsets"); } */
    r = exec_BW_alg(matches, secret_digest, shares, ia, server_keys.pk->modulus);
    ...
    InputArgs             ia)
{
    ...
    r = construct_shamir_shares(shares, bn_secret, server_pk.modulus, ia);
