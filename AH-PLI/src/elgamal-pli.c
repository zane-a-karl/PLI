			...
			char              *filename)
{
    ...
    if (!r) { perror("Failed to send modulus"); return FAILURE; }
    // 2nd: the generator
    ...
    if (!r) { perror("Failed to send mul_mask"); return FAILURE; }
	...
	if (htype == AH) {
	    ...
	    r &= mh_elgamal_encrypt(&server_cipher[i], *server_keys.pk, bn_plain[i]);
	}
	if (!r) { perror("Failed to encrypt bn_plain"); return FAILURE; }
	// Send C1
	...
	if (htype == AH) {
	    ...
	    r &= elgamal_skip_decrypt_check_equality(server_keys, client_cipher[i]);
	}
	if(!r) { perror("Failed skip decrypt check"); return FAILURE; }
    }
			...
			char *             filename)
{
    ...
    for (int i = 0; i < num_entries; i++) {
	if (htype == AH) {
	    r &= ah_elgamal_encrypt(&client_cipher[i], server_pk, bn_inv_plain[i]);
	} else {
	    r &= mh_elgamal_encrypt(&client_cipher[i], server_pk, bn_inv_plain[i]);
	}
	if (!r) { perror("Error encrypting bn_inv_plain"); return FAILURE; }
    }
	...
	if (!r) { perror("Failed to calc server_ciph * client_ciph"); return FAILURE; }
    }
	...
	if (!r) { perror("Failed to gen rand_exp"); return FAILURE; }
	printf("r[%i] = ", i);
	...
	if (!r) { perror("Failed to calculate cipher^mask"); return FAILURE; }
    }
	...
	if (!r) { perror("Failed to send exp_res.c1"); return FAILURE; }
	// Send C2
	r &= send_msg(sockfd, exp_res[i].c2, "client: sent exp_res.c2", Bignum);
	if (!r) { perror("Failed to send exp_res.c2"); return FAILURE; }
    }
