...
/*     snprintf(logfile, 32, "%s-%s-%d.%s", "logs/elgamal", "MH", sec_par, "csv"); \ */
    ...
    char              *filename)
{
    ...
    for (int i=0; i < num_entries; i++) {
	r = mh_elgamal_encrypt(&server_cipher[i], *server_keys.pk, bn_plain[i], sec_par);
    ...
    char *             filename)
{
    ...
    client_cipher = calloc(num_entries, sizeof(*client_cipher));
    for (int i = 0; i < num_entries; i++) {
	r = mh_elgamal_encrypt(&client_cipher[i], server_pk, bn_inv_plain[i], sec_par);
