...
if [[ $CUR_DIR != "BF-Paillier-PLI" ]];
then
    echo "You are in the wrong directory"
    echo "Please navigate to BF-Paillier-PLI/"
    exit 1
fi
if [[ $# -lt 1 ]];
then
...
sample_size=5
sec_par=$1
logfile="logs/bf-paillier-$sec_par.csv"
...
do
	...
	make --quiet clean;
	make --quiet;
	printf "%s%d\n" "Begin:  BF-Paillier PLI Protocol #" "$j"
	./bin/bf-paillier-client-and-server localhost MH $sec_par input/server.txt input/client.txt
	#	$pids=$(ps aux | grep "elgamal" | grep -v "grep" | awk '{print $2}')
	wait
	printf "%s%d\n\n" "Finish:  BF-Paillier PLI Protocol #" "$j"
    done
    echo "-------------------------------------------" >> "$logfile";
done
